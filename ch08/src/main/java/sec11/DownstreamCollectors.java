...
public class DownstreamCollectors {
    public static Stream<City> readCities(String filename) throws IOException, URISyntaxException {
        Path path = Paths.get(DownstreamCollectors.class.getResource(String.format("../%s", filename)).toURI());
        return Files.lines(path).map(l -> l.split(", ")).map(a -> new City(a[0], a[1], Integer.parseInt(a[2])));
    }
